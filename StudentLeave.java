...
import institutee.management.system.Conn;
...
public class StudentLeave extends JFrame implements ActionListener {
    ...
    StudentLeave() {
       ...
       setSize(470, 550);
        ...
        getContentPane().setBackground(new Color(54, 69, 79)); // Dark background color

        
        
 
        JLabel lblrollno = new JLabel("Search by Roll Number");
        lblrollno.setBounds(100, 90, 270, 30);
        lblrollno.setFont(new Font("SansSerif", Font.PLAIN, 18));
        lblrollno.setForeground(Color.WHITE);
        add(lblrollno);

        crollno = new Choice();
        crollno.setBounds(100, 120, 270, 40);
        add(crollno);
        ...
        lbltime.setBounds(100, 350, 270, 20);
        lbltime.setFont(new Font("SansSerif", Font.PLAIN, 18));
        add(lbltime);
        
        ctime = new Choice();
        ctime.setBounds(100, 390, 270, 40);
        ctime.add("Full Day");
        ctime.add("Half Day");
        add(ctime);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == submit) {
            ...
            String date2 = ((JTextField) dcdate.getDateEditor().getUiComponent()).getText();
            String duration = ctime.getSelectedItem();
            
            String query = "insert into studentleave values('"+rollno+"', '"+date+"','"+date2+"', '"+duration+"')";
